...
export default function ProposalsPage() {
  ...
  const handleLoadComplete = useCallback(() => {
    setIsLoading(false);
    setTimeout(() => setShowProposals(true), 600);
  }, []);
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {aiFactors.negative.map((factor) => (
                <div key={factor.id} className="flex items-start gap-3 p-3 rounded-lg border border-[var(--border)] bg-[var(--background)]">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-sm font-semibold text-[var(--foreground)]">{factor.title}</h4>
                      <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${factor.severity === "高" ? "badge-high" : "badge-medium"}`}>
                        {factor.severity}
                      </span>
                    </div>
                  </div>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {aiFactors.positive.map((factor) => (
                <div key={factor.id} className="flex items-start gap-3 p-3 rounded-lg border border-green-200 bg-green-50">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {proposals.map((proposal, index) => (
              <FadeIn key={proposal.id} delay={index * 200}>
                <div className="card">
